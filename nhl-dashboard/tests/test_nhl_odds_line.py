...
def _make_score_now(games=None, partners=None):
    ...
    return {"currentDate": "2026-05-24", "games": games or [], "oddsPartners": partners or []}


def _game_with_odds(game_id=_GAME_ID, away_odds=None, home_odds=None):
...
class TestNhlOddsLineModel:
    ...
    def test_nhl_odds_line_id_is_autoincrement_pk(self, db):
        ...
        r1 = NhlOddsLine(game_id=_GAME_ID, partner_id=7, fetched_at=now, away_value="-152", home_value="+126")
        r2 = NhlOddsLine(game_id=_GAME_ID, partner_id=9, fetched_at=now, away_value="-155", home_value="+130")
    ...
    def test_nhl_odds_line_fetched_at_not_null(self, db):
        ...
        _seed_partners(db)
        db.session.add(NhlOddsLine(game_id=_GAME_ID, partner_id=7, away_value="-152", home_value="+126"))
    ...
    def test_nhl_odds_line_decimal_format_values(self, db):
        ...
        db.session.add(NhlOddsLine(
            game_id=_GAME_ID, partner_id=3, fetched_at=now,
            away_value="1.67", home_value="2.24",
        ))
        db.session.commit()
...
class TestInsertOddsLines:
    def test_insert_odds_lines_inserts_one_row_per_partner(self, db):
        ...
        now = datetime.now(timezone.utc)
        _insert_odds_lines(_GAME_ID, away_odds, home_odds, now)
        db.session.commit()

        rows = db.session.scalars(select(NhlOddsLine)).all()
        assert len(rows) == 4
        assert {r.partner_id for r in rows} == {7, 9, 3, 6}
    ...
    def test_insert_odds_lines_deduplication_within_cooldown(self, db):
        ...
        _insert_odds_lines(_GAME_ID, away_odds, home_odds, t1)
        db.session.commit()

        t2 = t1 + timedelta(seconds=90)
        _insert_odds_lines(_GAME_ID, away_odds, home_odds, t2)
        db.session.commit()
    ...
    def test_insert_odds_lines_no_odds_inserts_nothing(self, db):
        ...
        from services.scores import _insert_odds_lines
        now = datetime.now(timezone.utc)
        _insert_odds_lines(_GAME_ID, [], [], now)
        db.session.commit()
    ...
    def test_insert_odds_lines_absent_odds_key_no_error(self, db):
        ...
        from services.scores import _insert_odds_lines
        now = datetime.now(timezone.utc)
        _insert_odds_lines(_GAME_ID, None, None, now)
        db.session.commit()
...
class TestRefreshNhlOddsIntegration:
    def test_refresh_nhl_odds_inserts_odds_lines_for_all_partners(self, db):
        """refresh_nhl_odds() inserts one NhlOddsLine row per partner when odds present."""
        _seed_partners(db, partner_ids=(7, 9))
        ...
        games = [_game_with_odds(game_id=_GAME_ID, away_odds=away_odds, home_odds=home_odds)]
    ...
    def test_refresh_nhl_odds_no_odds_no_error(self, db):
        """refresh_nhl_odds() handles games with empty odds arrays without error."""
        _seed_partners(db, partner_ids=(7,))

        games = [_game_with_odds(game_id=_GAME_ID, away_odds=[], home_odds=[])]
        api_data = _make_score_now(games=games)
...
class TestPruneNhlOddsLines:
    def test_prune_nhl_odds_lines_deletes_rows_older_than_30_days(self, db):
        ...
        assert db.session.scalars(select(NhlOddsLine)).all() == []
    ...
    def test_prune_nhl_odds_lines_retains_recent_rows(self, db):
        ...
        assert len(db.session.scalars(select(NhlOddsLine)).all()) == 1
    ...
    def test_prune_nhl_odds_lines_mixed_ages(self, db):
        ...
        now = datetime.now(timezone.utc)
        db.session.add_all([
            NhlOddsLine(game_id=_GAME_ID, partner_id=7,
                        fetched_at=now - timedelta(days=31),
                        away_value="-152", home_value="+126"),
            NhlOddsLine(game_id=_GAME_ID, partner_id=9,
