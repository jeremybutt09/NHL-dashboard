...
class TestPollNhlOddsSuccessLog:
    def test_poll_nhl_odds_success_emits_info_log(self, app, caplog):
        """_with_ctx(_poll_nhl_odds) emits at least one INFO record on success."""
        _set_app(app)
        with patch("services.scores.refresh_nhl_odds"):
            with caplog.at_level(logging.INFO, logger="scheduler"):
                sched._with_ctx(sched._poll_nhl_odds)()

        assert any(r.levelno == logging.INFO for r in caplog.records)

    def test_poll_nhl_odds_success_log_contains_job_name(self, app, caplog):
        ...
        assert any("_poll_nhl_odds" in m or "poll_schedule" in m for m in info_messages)


# ── failed run emits ERROR with traceback ────────────────────────────────────

class TestPollNhlOddsErrorLog:
    def test_poll_nhl_odds_exception_emits_error_log(self, app, caplog):
        """_with_ctx(_poll_nhl_odds) emits an ERROR record when refresh_schedule raises."""
        _set_app(app)
        with patch("services.scores.refresh_nhl_odds", side_effect=RuntimeError("nhl api down")):
            with caplog.at_level(logging.ERROR, logger="scheduler"):
                sched._with_ctx(sched._poll_nhl_odds)()

        assert any(r.levelno == logging.ERROR for r in caplog.records)

    def test_poll_nhl_odds_exception_log_contains_job_name(self, app, caplog):
        ...
        assert any("_poll_nhl_odds" in r.message or "poll_schedule" in r.message for r in error_records)

    def test_poll_nhl_odds_exception_includes_exc_info(self, app, caplog):
    ...
    def test_poll_nhl_odds_exception_does_not_propagate(self, app):
